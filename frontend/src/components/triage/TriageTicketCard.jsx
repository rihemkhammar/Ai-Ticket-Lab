...
export default function TriageTicketCard({ item }) {
  ...
  const handleViewTrace = async () => {
    ...
    try {
      ...
      setTraceError(err.response?.data?.message ?? 'Unable to load trace.');
      setTraceOpen(true);
    } finally {
  ...
  const handleOpenHitl = async () => {
    ...
    try {
      ...
      setHitlError(err.response?.data?.message ?? 'Unable to load the HITL review.');
      setHitlOpen(true);
    } finally {
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          {pending && (
            <span style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: '#facc15' }}>
              <TbAlertTriangle size={12} /> Review required
            </span>
          )}
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '10px' }}>
            ...
            {pending && item.agentRunId && (
              <button onClick={handleOpenHitl} disabled={hitlLoading} style={{
                ...
                {hitlLoading ? 'Loading…' : `Open human review — ${HITL_RUN_STATUS_CONFIG.WAITING_FOR_HUMAN.label}`}
              </button>
            )}
          </div>
        </div>
